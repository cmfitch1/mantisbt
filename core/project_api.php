...
function project_get_all_user_rows( $p_project_id = ALL_PROJECTS, $p_access_level = ANYBODY, $p_include_global_users = true, $p_include_disabled_users = false ) {
	...
	if( $p_include_global_users ) {
		...
		if( is_array( $t_global_access_level ) ) {
			if( empty( $t_global_access_level ) ) {
				$t_where = 'access_level >= ' . $t_query->param( NOBODY );
			} else {
				$t_where = $t_query->sql_in( 'access_level', $t_global_access_level );
			}
		} else {
			$t_where = 'access_level >= ' . $t_query->param( (int)$t_global_access_level );
		}
		if ( !$p_include_disabled_users ) {
			$t_where .= ' AND enabled = ' . $t_query->param( true );
		}
		$t_sql = 'SELECT id, username, realname, access_level FROM {user} WHERE ' . $t_where;
		$t_query->sql( $t_sql );
	...
	if( $c_project_id != ALL_PROJECTS ) {
		...
		$t_sql = 'SELECT u.id, u.username, u.realname, l.access_level FROM {project_user_list} l'
				. ' JOIN {user} u ON l.user_id = u.id'
				. ' WHERE l.project_id = :project';
		if ( !$p_include_disabled_users ) {
			$t_sql .= ' AND u.enabled = :enabled';
		}
		$t_params = array( 'project' => $c_project_id, 'enabled' => true );
